...
def test_client_model(X_test, Y_test,  model,pp_Group,npp_Group,  sa_index, fairness_notion, ytest_potential, s_client_list, sensitive_feature):
    ...
    if fairness_notion == 'stat_parity':
        ...
        x_test_client_df = pd.DataFrame(X_test, columns=features)
        s_client = x_test_client_df[sensitive_feature]
        s_client_list = s_client.tolist()
        ytest_potential = find_potential_outcomes(x_test_client_df,Y_test, sensitive_feature)
        '''
